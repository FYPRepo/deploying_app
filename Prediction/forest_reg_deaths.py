...
def get_dataframe(country, case_type):
    ...
    pivot1 = pd.pivot_table(df_deaths,
                            values=country,
                            index='Date').sort_values(by='Date')

    deaths_final = pd.DataFrame(pivot1)

    deaths_final.reset_index(inplace=True)

    Deaths = deaths_final[['Date', country]]

    df_d = Deaths
    df_d['Deaths'] = df_d[country]

    df_d['Deaths_per_day'] = df_d['Deaths'].diff()
    df_d = df_d.fillna(0)
    df_d['Deaths_per_day'] = df_d['Deaths_per_day'].astype(int)

    if case_type == 2:
        columns = ['Date', 'Deaths_per_day']
    else:
        columns = ['Date', 'Deaths']
...
# next_day_my_deaths_day = forest_reg('Malaysia', 2)
# next_day_my_deaths_cumm = forest_reg('Malaysia', 1)
#
# print("====== next day deaths per day ====")
# print(next_day_my_deaths_day)
# print("===================================\n")
#
# print("====== next day deaths cumm ====")
# print(next_day_my_deaths_cumm)
# print("===================================")
